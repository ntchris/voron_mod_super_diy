# ERCF V2 selector mod for larger 28mm (17g) servo Verson2


## New changes in Version2 VS version1
  * In version1 servo arm cannot touch the filament block top hat to release brake. The work around for ver1 is to remove the brake nut screw and file the brake part hole larger.
  * In version2 , move the whole selector part to left about 5mm, so it has enough room for 28mm servo, the servo arm is in the same place as the 23mm servo in original design.
  * since the 28mm servo is in the same place as 23mm orig servo, it now can touch the filament block top hats, this can release the break, to let filament freely move. 
  * since the whole selector part is 5mm lefter, the encoder case right half need to be longer to reach the selector output hole.
